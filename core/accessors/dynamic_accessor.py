...
from core.memory import External2DMemory, External3DMemory
...
class DynamicAccessor3D(Accessor):
    def __init__(self, args):
        super(DynamicAccessor3D, self).__init__(args)
        # logging
        self.logger = args.logger
        # params
        self.use_cuda = args.use_cuda
        ...
        self.logger.warning("<--------------------------------===> Accessor:   {WriteHead3D, ReadHead3D, 3DMemory}")

        # functional components
        self.usage_vb = None    # for dynamic allocation, init in _reset
        self.link_vb = None     # for temporal link, init in _reset
        self.preced_vb = None   # for temporal link, init in _reset
        self.write_heads = WriteHead(self.write_head_params)  # TODO: 3d write/read heads
        self.read_heads = ReadHead(self.read_head_params)
        self.memory = External3DMemory(self.memory_params)
